...
namespace IX.Math.Nodes.Operations.Function.Binary
{
    ...
    {
        public FunctionNodeTrimBody(NodeBase stringParameter, NodeBase numericParameter)
        ...
        public override NodeBase DeepClone(NodeCloningContext context) => new FunctionNodeTrimBody(this.FirstParameter.DeepClone(context), this.SecondParameter.DeepClone(context));

        public override NodeBase Simplify()
        {
            if (this.FirstParameter is StringNode stringParam && this.SecondParameter is StringNode charParam)
            {
                return new StringNode(stringParam.Value.Replace(charParam.Value, string.Empty));
            }
        ...
        protected override Expression GenerateExpressionInternal()
        {
            ...
            return Expression.Call(e1, mi, e2, Expression.Constant(string.Empty, typeof(string)));
        }
    }
}