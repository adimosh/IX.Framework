...
#pragma warning disable HAA0301 // Closure Allocation Source
        ...
        public void SetupStandaloneOpenRead(
            string fileName,
            string fileContents) => this.SetupStandaloneOpenRead(
            fileName,
            fileContents,
            Encoding.UTF8);
        ...
        public void SetupStandaloneOpenRead(
            string fileName,
            byte[] fileContents) =>
            this.Setup(p => p.OpenRead(It.Is<string>(q => q == fileName)))
                .Returns(() => new MemoryStream(fileContents));
        ...
        public void SetupStandaloneOpenText(
            string fileName,
            string fileContents) => this.SetupStandaloneOpenText(
            fileName,
            fileContents,
            Encoding.UTF8);
        ...
        public void SetupStandaloneOpenText(
            string fileName,
            byte[] fileContents) =>
            this.Setup(p => p.OpenText(It.Is<string>(q => q == fileName)))
                .Returns(() => new StreamReader(new MemoryStream(fileContents)));
        ...
        /// <param name="fileName">The name of the file to set up the mock for.</param>
        /// <returns>
        ///     A <see cref="WriteMockWaiter" /> that can be used to determine when the file writing operation has finished
        ///     and to assert on the resulting contents.
        /// </returns>
